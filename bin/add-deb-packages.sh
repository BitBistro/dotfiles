...
case "$LEVEL" in
    ...
    "standard")
        aptitude -r install '?and(?architecture(native),?or(~prequired,~pimportant,~pstandard),?not(~v),?not(~slibs))' \
                ...
                dbus dbus-user-session ripgrep tree yq
        aptitude unmarkauto '?and(?architecture(native),?or(~prequired,~pimportant,~pstandard),?not(~v),?not(~slibs),~i)' \
                ...
                tcpdump wget wodim busybox-static pinentry-fltk pass dbus dbus-user-session ripgrep tree yq
    ;;
    "extra")
        aptitude install \
    ...
    "audit")
        EXPLICIT_LIST=(
            ...
            wodim busybox-static pinentry-fltk pass dbus dbus-user-session ripgrep tree yq
