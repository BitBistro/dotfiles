...
color_prompt=
case "$TERM" in
  xterm-color|*-256color|linux|iTerm*|iterm*)
    color_prompt=yes;;
  *)
...
if [ "$color_prompt" = yes ]; then
  ...
  SWITCH=00
  if [ -x /usr/bin/tput ] && [ `tput colors` -gt 16 ]; then
      SWITCH=01
  fi
      
  # Sets up a super fancy color prompt that includes the git variables.
  PS1='${prompt_context:+"(\[\033[0m\]${prompt_context}\[\033[00m\]) "}'
  PS1="${PS1}"'\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033['$SWITCH';34m\]\w'
